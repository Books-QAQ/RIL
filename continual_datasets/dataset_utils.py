...
def open_maybe_compressed_file(path):
    """
    Accepts either a filesystem path (str/bytes/os.PathLike) or an already-opened
    binary file object. Returns a readable binary file-like object.
    """
    # 如果传入的是文件对象，直接返回
    if not isinstance(path, (str, bytes, os.PathLike)):
        return path

    p = os.fspath(path)  # 统一成字符串路径
    if p.endswith(".gz"):
