...
class Engine():
    ...
    def inference_acc(self, model, data_loader, device):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                # ✅ Support (input, target) OR (input, target, domain, ...)
                ...
                if output.shape[-1] > self.num_classes:
                    output, _, _ = self.get_max_label_logits(output, self.current_classes)
                ...
                for i, label in enumerate(self.current_classes):
                    m = (target == label)
                    correct_pred_per_label[i] += torch.sum(correct_predictions[m]).item()
                    num_instance_per_label[i] += m.sum().item()

        for correct, num in zip(correct_pred_per_label, num_instance_per_label):
            accuracy_per_label.append(round(correct / num, 2) if num > 0 else 0.0)  # avoid div0
    ...
    def train_one_epoch(self, model: torch.nn.Module,
        ...
        for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
            input, target, *rest = batch  # rest 里是多出来的内容（task_id/index等），不影响原逻辑
            ...
            if self.distill_head != None:
                ...
                m = torch.isin(torch.tensor(self.labels_in_head[cur_class_nodes]),
