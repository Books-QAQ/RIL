...
class Engine():
    def __init__(self, model=None, device=None, class_mask=[], domain_list=[], args=None):
        ...
        # [Migrated] 初始化 global_step 用于 step-level checkpoint
        self.global_step = 0
    ...
    # [Migrated] 新增方法：保存 adapter step-level checkpoint
    def _save_adapter_step_ckpt(self, model, args, task_id: int, epoch: int, step_in_epoch: int):
        """
        Save adapter-only (and optionally head) checkpoint for batch-level analysis.
        File name includes task_id and global_step so you can reconstruct timeline later.
        """
        ...
        # folder
        step_dir = os.path.join(args.output_dir, "checkpoint_step")
        Path(step_dir).mkdir(parents=True, exist_ok=True)

        # only keep adapter/head params to reduce size
        full_sd = model.state_dict()
        if getattr(args, "save_adapter_only", False):
            kept = {}
            ...
            state_dict = {"model": {k: v.detach().cpu() for k, v in full_sd.items()}}
        ...
        ckpt_path = os.path.join(
            step_dir,
            f"task{task_id + 1}_ep{epoch + 1}_step{self.global_step}.pth"
        )
        utils.save_on_master(state_dict, ckpt_path)
    ...
    def get_max_label_logits(self, output, class_mask, task_id=None, slice=False, target=None):
        # [Fix] 获取当前模型输出的物理宽度
        max_dim = output.shape[1]
        device = output.device

        def _agg(x, idx_tensor, label):
            # idx_tensor 必须是 LongTensor 且在 device 上
            if idx_tensor.numel() == 1:
                return x[:, idx_tensor[0]]
            # dim=1 聚合
            return x[:, idx_tensor].max(dim=1)[0]

        for label in range(self.num_classes):
            # [LHS Check] 写入保护：如果目标列超出 output 宽度，绝对不能写
            if label >= max_dim:
                continue

            # 获取该 label 对应的所有 head 节点索引
            label_nodes = np.where(self.labels_in_head == label)[0]
            if len(label_nodes) == 0:
                continue

            # [RHS Check] 读取保护：过滤掉所有超出 max_dim 的节点索引
            # 强制转换为 list -> tensor，避免 numpy 与 cuda 的潜在兼容问题
            valid_nodes_list = [idx for idx in label_nodes if idx < max_dim]

            if len(valid_nodes_list) == 0:
                continue

            # [Explicit Convert] 显式转换为 GPU Tensor
            valid_nodes_tensor = torch.tensor(valid_nodes_list, dtype=torch.long, device=device)

            # 执行聚合
            # output[:, label] 是安全的 (因为 label < max_dim)
            # valid_nodes_tensor 是安全的 (因为所有元素都 < max_dim)
            output[:, label] = _agg(output, valid_nodes_tensor, label)

        if slice:
            # [Slice Check] 确保切片范围不超过物理宽度
            slice_limit = min(self.num_classes, max_dim)
            output = output[:, :slice_limit]
    ...
    def train_one_epoch(self, model: torch.nn.Module,
        ...
        for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
            max_b = int(getattr(args, "max_batches_per_task", 0) or 0)
            ...
            # [安全检查] 获取当前 output 的实际物理宽度
            curr_out_dim = output.shape[1]

            # Distillation logic (蒸馏逻辑)
            distill_loss = 0
            if self.distill_head is not None:
                feature = model.forward_features(input)[:, 0]
                output_distill = self.distill_head(feature)

                # [Fix] 获取旧模型(Teacher)的实际宽度
                distill_dim = output_distill.shape[1]
                ...
                distill_cols = cur_class_nodes[~m_added]

                # [Critical Fix] 过滤掉超出旧模型宽度的索引！
                # 只有在旧模型中也存在的节点，才能进行蒸馏
                distill_cols = distill_cols[distill_cols < distill_dim]

                if distill_cols.numel() > 0:
                    distill_loss = self.kl_div(output[:, distill_cols], output_distill[:, distill_cols])

            # --- Logits 聚合与复制 ---
            # 只有当实际维度大于 num_classes (说明有多节点聚合需求) 时才执行
                    # --- Logits Masking & Slicing (在 train_one_epoch 中) ---
                    if curr_out_dim > self.num_classes:
                        ...
                        if len(self.added_classes_in_cur_task) > 0:
                            for added_class in self.added_classes_in_cur_task:
                                # 查找该 class 对应的节点
                                node_indices = np.where(self.labels_in_head == added_class)[0]
                                if len(node_indices) > 0:
                                    cur_node = node_indices[-1]  # 取最新的一个
                                    # [Double Check] 确保 目标列(added_class) 和 源列(cur_node) 都存在
                                    if added_class < curr_out_dim and cur_node < curr_out_dim:
                                        output[:, added_class] = output[:, cur_node]

                # 切片时也要尊重 max_dim
                slice_dim = min(self.num_classes, curr_out_dim)
                output = output[:, :slice_dim]
            ...
            def _mask_logits(logits_x, allow_set):
                if logits_x is None: return None

                actual_dim = logits_x.shape[1]
                allow = np.array(sorted(list(allow_set)), dtype=np.int64)
                # 过滤掉超出当前维度的 allow 索引
                allow = allow[allow < actual_dim]

                all_ids = np.arange(actual_dim, dtype=np.int64)
                not_mask = np.setdiff1d(all_ids, allow)

                if not_mask.size == 0: return logits_x
            ...
            if use_la_train:
                counts = torch.tensor(np.maximum(self.cum_counts_total, 1), dtype=torch.float32, device=device)
                # 对齐 counts 和 logits 的维度
                if counts.shape[0] > logits_new.shape[1]:
                    counts = counts[:logits_new.shape[1]]
                elif counts.shape[0] < logits_new.shape[1]:
                    # 如果 counts 比 logits 短（少见情况），进行填充
                    pad = torch.ones(logits_new.shape[1] - counts.shape[0], device=device)
                    counts = torch.cat([counts, pad])
            ...
            self.global_step += 1
            save_every = int(getattr(args, "save_adapter_every", 0) or 0)
            if save_every > 0 and (self.global_step % save_every == 0):
                self._save_adapter_step_ckpt(
                    model=model,
                    args=args,
                    task_id=task_id,
                    epoch=epoch,
    ...
    def train_and_evaluate(self, model: torch.nn.Module, criterion, data_loader: Iterable,
        ...
        for task_id in range(args.num_tasks):
            ...
            if hasattr(model, 'head'):
                head_dim = model.head.weight.shape[0]
                tracker_dim = len(self.labels_in_head)
                if head_dim != tracker_dim:
                    print(f"\n[Critical Warning] Dimension Mismatch detected after pre_train_task!")
                    print(f"  Model Head Dim: {head_dim}")
                    print(f"  Engine Tracker Dim: {tracker_dim}")
