...
from dataset_all import build_continual_dataloader, export_task_plan_txt
from engine_all import Engine
import models
import utils
import os
...
def set_data_config(args):
    if args.dataset == "iDigits":
        ...
        args.domain_num = 8
    return args
# --- DEBUG: verify if dataloader batches contain domain ---
# def debug_one_batch_per_task(loader_dicts, max_tasks=4):
#     import torch
#
#     print("\n[DEBUG] Verify batch format per task (train loader)")
#     for tid, pack in enumerate(loader_dicts[:max_tasks]):
#         dl = pack["train"]
#         batch = next(iter(dl))
#
#         # batch should be (x, y) or (x, y, domain) or (x, y, domain, ...)
#         if not isinstance(batch, (list, tuple)):
#             print(f"task {tid}: unexpected batch type = {type(batch)}")
#             continue
#
#         print(f"task {tid}: len(batch) = {len(batch)}")
#
#         x = batch[0]
#         y = batch[1]
...
#                 print(f"  d.shape={tuple(d.shape)}, unique domains={torch.unique(d).tolist()}")
#             else:
#                 print(f"  domain exists but is not a tensor: type(d)={type(d)} value={d}")
#         else:
#             print("  NO DOMAIN (only (x, y))")
#
#     print("[DEBUG] Done.\n")
...
def main(args):
    ...
    # debug_one_batch_per_task(data_loader, max_tasks=min(4, len(data_loader)))
    ...
    # print("args.use_replay =", args.use_replay)
    # print("args.use_la_train =", args.use_la_train)
    # print("args.classwise_tau =", args.classwise_tau)
    # print("args.replay_forget =", args.replay_forget)
...
if __name__ == '__main__':
    ...
    # Data parameters
    parser.add_argument('--data-path', default='/local_datasets/', type=str, help='dataset path')# /local_datasets    /root/VIL/datasets/UIL/VIL
    ...
    parser.add_argument('--thre', default=0.0, type=float,help='value of static threshold if not using dynamic thresholding')
    ...
    # IC_Gating
    parser.add_argument('--pooler', type=str, default='max', choices=['max'],help='多节点聚合方式（IC） max')
    parser.add_argument('--evidence_gating', action='store_true', default=False,help='是否开启频率门控（Evidence Gating），拦截低频类别的扩容')

    # main_all.py

    # 在 IC / CAST 参数附近
    parser.add_argument('--use_sce', action='store_true', default=True,
                        help='Enable Semantic-Consistency Expansion (Feature Drift Detection)')
    parser.add_argument('--sce_sensitivity', type=float, default=1.20,
                        help='Sensitivity of SCE. Higher value means easier expansion on drift.')
