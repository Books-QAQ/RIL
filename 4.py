...
def plot_per_task_sims_boxplot(
    ril_stats,
    vil_stats,
    save_path="per_task_sims_boxplot.png",
    seed=0,
    ...
    common = set(ril_stats.keys()) & set(vil_stats.keys())
    common = [t for t in common if ril_stats[t]["K"] > 0 and vil_stats[t]["K"] > 0]
    ...
    vil_data = [np.asarray(vil_stats[t]["sims"], dtype=np.float64) for t in tasks]
    ril_data = [np.asarray(ril_stats[t]["sims"], dtype=np.float64) for t in tasks]
    ...
    plt.figure(figsize=(max(12, 0.55 * len(tasks) + 4), 5.2))
    ...
    # Optional: print mapping index -> original task id
    mapping = ", ".join([f"{i}->T{t+1}" for i, t in enumerate(tasks)])
    print(f"[Index mapping] {mapping}")


if __name__ == "__main__":
    RIL_PATH = "/root/VIL_main13/output4/output_ril_Do_421"
    VIL_PATH = "/root/VIL_main13/output4/output_vil_Do_421"
    ...
    ril = per_task_sims_and_strength(
        RIL_PATH, device="cpu", epoch1_only=epoch1_only, epoch1_index=epoch1_index, do_debug=do_debug
    )
    vil = per_task_sims_and_strength(
        VIL_PATH, device="cpu", epoch1_only=epoch1_only, epoch1_index=epoch1_index, do_debug=do_debug
    )

    common_tasks = sorted(set(ril.keys()) & set(vil.keys()))
    common_tasks = [t for t in common_tasks if ril[t]["K"] > 0 and vil[t]["K"] > 0]
    if not common_tasks:
        raise RuntimeError("No common tasks with valid K to compare.")

    tau = compute_global_tau_from_vil_quantile(vil, common_tasks, q=q)
    print(f"\n=== Global tau from VIL quantile q={q:.2f}: tau={tau:.6f} ===\n")

    ril = add_conflict_metrics(ril, tau=tau, a=a)
    vil = add_conflict_metrics(vil, tau=tau, a=a)

    for t in common_tasks:
        print(
            f"T{t+1:02d}  RIL mean={ril[t]['strength_mean']:.4f} (K={ril[t]['K']})   "
            f"VIL mean={vil[t]['strength_mean']:.4f} (K={vil[t]['K']})   "
            f"conf_rate(RIL)={ril[t]['conflict_rate']:.3f}  conf_rate(VIL)={vil[t]['conflict_rate']:.3f}"
        )

    # 图1：conflict_rate（保留全量common task，按T1..顺序）
    plot_conflict_rate_bar_with_global_avg(ril, vil, key="conflict_rate", save_path="per_task_conflict_rate.png")

    # 图2：只画你指定的10个task，并按你给的顺序；x轴显示0~9
    ...
    TASK_ORDER = [t - 1 for t in TASK_ORDER_1BASED]  # convert to 0-based

    plot_per_task_sims_boxplot(
        ril, vil,
        save_path="per_task_sims_boxplot.png",
